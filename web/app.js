...
const STORAGE_KEY = 'preventivi-clone-ai-desktop-beta6-2-7-hybrid'
const LEGACY_STORAGE_KEYS = ['preventivi-clone-ai-prova-094', 'preventivi-clone-ai-prova-093', 'preventivi-clone-ai-prova-092', 'preventivi-clone-ai-prova-09', 'preventivi-clone-ai-prova-083', 'preventivi-clone-ai-prova-082', 'preventivi-clone-ai-prova-08', 'preventivi-clone-ai-prova-07', 'preventivi-clone-ai-prova-06', 'preventivi-clone-ai-prova-05', 'preventivi-clone-ai-prova-04']
...
function effectiveGrossTotal(row) {
  if (row.kind === 'item' && calculatedChildrenFor(row.id).length) return 0
...
function createTextarea(value, rows, onInput) {
...
function getRowDisplayTotal(row) {
  if (row.kind === 'item') {
    const pricedChildren = calculatedChildrenFor(row.id)
...
function addSubRow(parentOrChild, calculable = true) {
  ...
  if (calculable && parent.calculate) {
    parent.calculateBeforeComponents = true
    parent.calculate = false
  }
  const row = {
...
function renderRows() {
  ...
  visibleRows.forEach((row, visibleIndex) => {
    ...
    if (row.kind === 'section') {
      ...
      const td = document.createElement('td')
      td.colSpan = 8
      td.append(createInput(row.description, value => updateRow(row.id, { description: value })))
    ...
    if (children.length) {
      const pricedChildren = children.filter(candidate => candidate.calculate)
    ...
    unitTd.append(createInput(isNote ? '' : row.unit, value => updateRow(row.id, { unit: value.toUpperCase() }), { className: 'unit-input', disabled: isNote }))

    const quantityTd = document.createElement('td')
    quantityTd.append(createInput(isNote ? '' : (row.quantity ?? ''), value => {
      row.quantity = value === '' ? null : Number(value)
      save(); renderRows()
    }, { type: 'number', step: '0.0001', className: 'number-input', disabled: isNote, eventName: 'change' }))

    const priceTd = document.createElement('td')
    priceTd.append(createInput(row.unitPrice ?? 0, value => {
      row.unitPrice = Number(value) || 0
      save(); renderRows()
    }, { type: 'number', step: '0.01', className: 'number-input', disabled: isNote || !row.calculate, eventName: 'change' }))
...
async function deleteRow(row) {
  ...
  if (parentId) {
    ...
    if (parent && !calculatedChildrenFor(parent.id).length && parent.calculateBeforeComponents) {
