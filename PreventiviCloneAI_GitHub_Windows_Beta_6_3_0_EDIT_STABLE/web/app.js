...
const STORAGE_KEY = 'preventivi-clone-ai-desktop-beta6-3-0-edit-stable'
...
function refreshCalculatedValuesInPlace(changedRowId = '') {
  const updateRowDisplay = row => {
    const tr = [...el.rowsBody.querySelectorAll('tr[data-id]')].find(candidate => candidate.dataset.id === row.id)
    if (!tr || row.kind === 'section' || row.kind === 'total') return
    const isNote = row.kind === 'note'
    const netCell = tr.querySelector('.net-cell')
    const totalCell = tr.querySelector('.total-cell')
    if (netCell) netCell.textContent = (!isNote && row.calculate) ? formatCurrency(netUnitPrice(row)) : '—'
    if (totalCell) totalCell.textContent = isNote ? '—' : formatCurrency(getRowDisplayTotal(row))

    const priceInput = tr.querySelector('[data-field-key="unitPrice"]')
    const discountInput = tr.querySelector('[data-field-key="discountPct"]')
    if (priceInput) priceInput.disabled = isNote || !row.calculate
    if (discountInput) discountInput.disabled = isNote || !row.calculate
  }

  if (changedRowId) {
    const changed = state.rows.find(row => row.id === changedRowId)
    if (changed) updateRowDisplay(changed)
    if (changed?.parentId) {
      const parent = state.rows.find(row => row.id === changed.parentId)
      if (parent) {
        updateRowDisplay(parent)
        const parentTr = [...el.rowsBody.querySelectorAll('tr[data-id]')].find(candidate => candidate.dataset.id === parent.id)
        const badge = parentTr?.querySelector('.group-badge')
        if (badge) {
          const children = childRowsFor(parent.id)
          const calculatedChildren = children.filter(candidate => candidate.calculate)
          const childTotal = calculatedChildren.reduce((sum, candidate) => sum + ownRowTotal(candidate), 0)
          badge.textContent = `${children.length} sottovoci · ${calculatedChildren.length} calcolate · ${formatCurrency(childTotal)}`
        }
      }
    }
  } else {
    state.rows.forEach(updateRowDisplay)
  }

  state.rows.filter(row => row.kind === 'total').forEach(row => {
    const tr = [...el.rowsBody.querySelectorAll('tr[data-id]')].find(candidate => candidate.dataset.id === row.id)
    const totalCell = tr?.querySelector('.calculated-total')
    if (totalCell) totalCell.textContent = formatCurrency(sectionTotalFor(row.sectionId))
  })
  renderTotals()
}
...
function renderRows() {
  ...
  visibleRows.forEach((row, visibleIndex) => {
    ...
    if (children.length) {
      ...
      group.textContent = `${children.length} sottovoci · ${calculatedChildren.length} calcolate · ${formatCurrency(childTotal)}`
      meta.append(group)
    }
